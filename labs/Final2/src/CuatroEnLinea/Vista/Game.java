...
public class Game extends JFrame {
	...
	String[] selector = new String[3]; /*ESCOGER LA IMAGEN DEL JUGADOR
										*/
	String urlRevers = "C:/Users/preci/Music/Final2/src/images/revers.jpg";
	Winner winner = new Winner();
	/*
	 * INICIAN LOS DATOS QUE LA MAQUINA USA PARA DETERMINAR DONDE JUGAR
	 */
	...
	/*
	 * PARA BLOQUEAR FLIAS Y COLUMNAS 
	 */
	boolean p1 = true;
	boolean p2 = true;
	boolean p3 = true;
	...
	/*
	 * REINICIA LOS DATOS DE LA MAQUINA
	 */
	public void vaciar() { 
	...
	/* DIBUJA DONDE JUEGA LA MAQUINA Y ACTUALIZA LOS DATOS DEL JUEGO ATRAVEZ DE OTRAS FUNCIONES
	 * <b>post:</b> desbloquea lugares osibles donde puede jugar la maquina y revisa si la maquina gana
	 * @param i, es la fila donde se van a realizar los cambios y j es la columna, comenzando de abajo para arriba y de izquierda a derecha, para llenar los datos
	 */
	...
	public void logica(int i, int j) {
		...
		while (!a) { //si ya dibujo ficha se deja de ejecutar de lo contrario ejecutese hasta dibujar
			...
			if (random == 1 & !a) { //dibujar en cualquier posicion valida
				int temp[], temp2[];
			...
			if (random == 2 & !a) {//izquierda
				if (j < 6 & j > 0) {
					if (!bloqueo[i][j - 1] & !ocupado[i][j - 1]) {
						a = pc(i, j - 1);
					} else { //para evitar dibujar arriba se ejecute mas seguido que las otras opciones
						random = 3;
					}
				}
				if (j == 6) {
			...
			if (random == 4 & !a) {//diagonal izquierda
				if (i != 5 & j != 0) {
					if (ocupado[i][j - 1]) {
			...
			if (random == 5 & !a) { //diagonal derecha
				if (i != 5 & j != 6) {
					if (ocupado[i][j + 1]) {
						if (!bloqueo[i + 1][j + 1] & !ocupado[i + 1][j + 1]) {
							a = pc(i + 1, j + 1);
	 ...
	 * @return true si se dibuja una ficha
	 */
		...
		if (!a) {
			...
			if (i >= 2 & !a) {
				if (ocupado[i][j] & ocupado[i - 1][j] & ocupado[i - 2][j]) { // winner
																				// abajo-arriba
					if ((color[i][j]) & (color[i - 1][j]) & (color[i - 2][j])) { // 3 fichas jugador seguidas
			...
			if (i < 5 & i > 1 & j < 6 & j > 1 & !a) {
				if (ocupado[i][j] & ocupado[i - 1][j - 1]
						& ocupado[i - 2][j - 2]) { // diagonal derecha winner
					if ((color[i][j]) & (color[i - 1][j - 1])
							& (color[i - 2][j - 2])) { // 3 fichas jugador seguidas
			...
			if (i < 5 & i > 1 & j > 0 & j < 5 & !a) {
				if (ocupado[i][j] & ocupado[i - 1][j + 1]
						& ocupado[i - 2][j + 2]) { // diagonal izquierda winner
					if ((color[i][j]) & (color[i - 1][j + 1])
	...
	/*
	 * LA PC BUSCA COMPLETAR 4 FICHAS SEGUIDAS
	 * return true si se ejecua alguna ficha
	 */
		...
		for (int i = 0; i < 6; i++) {
			for (int j = 0; j < 7; j++) {
				if (!a) {
					...
					if (j > 2) {//compara las posiciones de lado izquierda a derecha
						if (ocupado[i][j] & ocupado[i][j - 1]
								& ocupado[i][j - 2]) {
							if ((!color[i][j]) & (!color[i][j - 1])
									& (!color[i][j - 2])) { // 3 fichas PC seguidos
								if (!ocupado[i][j - 3] & !bloqueo[i][j - 3]) {
									a = pc(i, j - 3);//lado izquierdo
								}
							}
						}
					}
					if (i >= 2) { //solo es posible ganar de esta forma en este espacio
						if (ocupado[i][j] & ocupado[i - 1][j]
								& ocupado[i - 2][j]) {
							if ((!color[i][j]) & (!color[i - 1][j])
									& (!color[i - 2][j])) { //  3 fichas PC seguidos
								if (!ocupado[i + 1][j] & !bloqueo[i + 1][j]) {
									a = pc(i + 1, j); //colocar la ficha arriba
								}
							}
						}
					}
					if (i < 5 & i > 1 & j < 6 & j > 1) {//solo es posible ganar de esta forma en un espacio
						if (ocupado[i][j] & ocupado[i - 1][j - 1]
								& ocupado[i - 2][j - 2]) { // diagonal derecha winner
							if ((!color[i][j]) & (!color[i - 1][j - 1])
									& (!color[i - 2][j - 2])) {  //3 fichas PC seguidos
								if (!ocupado[i + 1][j + 1]
										& !bloqueo[i + 1][j + 1]) {
									a = pc(i + 1, j + 1);
								}
					...
					if (i < 5 & i > 1 & j > 0 & j < 5) { //solo es posible ganar de esta forma en un espacio
						if (ocupado[i][j] & ocupado[i - 1][j + 1]
								& ocupado[i - 2][j + 2]) { // diagonal izquierda winner
							if ((!color[i][j]) & (!color[i - 1][j + 1])
									& (!color[i - 2][j + 2])) { // 3 fichas PC seguidos
	...
	/*
	 * DIBUJA LA FICHA EN LA POSICION QUE SE ANALIZO
	 */
			...
			for (int j = 0; j < 7; j++) {
				if (j < 4 & b) {
					if (ocupado[i][j] & ocupado[i][j + 1] & ocupado[i][j + 2]
							& ocupado[i][j + 3]) { // winer en lateral de izquierda a derecha
						if ((!color[i][j]) & (!color[i][j + 1])
								& (!color[i][j + 2]) & (!color[i][j + 3])) { // 4 fichas de pc seguidas
							b = false;
							terminar.Botones();
				...
				if (j > 2 & b) {
					if (ocupado[i][j] & ocupado[i][j - 1] & ocupado[i][j - 2]
							& ocupado[i][j - 3]) { // winer en lateral de derecha a izquierda
						if ((!color[i][j]) & (!color[i][j - 1])
								& (!color[i][j - 2]) & (!color[i][j - 3])) { // 4 ficahs de PC seguidos
							terminar.Botones();
							b = false;
							Resultados vr = new Resultados();
							vr.setNombreJ("Destructor");
				...
				if (i < 3 & b) {
					if (ocupado[i][j] & ocupado[i + 1][j] & ocupado[i + 2][j]
							& ocupado[i + 3][j]) { // winner arriba de abajo a arriba
						if ((!color[i][j]) & (!color[i + 1][j])
								& (!color[i + 2][j]) & (!color[i + 3][j])) { // 4 fichas de pc seguidos
							terminar.Botones();
							b = false;
							Resultados vr = new Resultados();
							vr.setNombreJ("Destructor");
	...
	public Game() {
		...
		lblJugador.setBounds(713, 28, 51, 14);
		...
		lbNombreJ1.setBounds(625, 28, 90, 16);
		...
		lblComputadora.setBounds(10, 28, 73, 14);
		...
		lbNombreComp.setBounds(93, 28, 89, 16);
		...
		rdbtnJC.setBounds(188, 24, 109, 23);
		...
		// ES EL BOTON QUE ABRE LA VENTANA DE LOS RESULTADOS
		...
		btnResultados.setIcon(new ImageIcon(Game.class
